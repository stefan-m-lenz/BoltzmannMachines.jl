...
function fitpartdbmcore(x::Array{Float64,2},
   ...
   for i=1:length(nhiddens)
      ...
      curout = nhiddens[i]
      weights = randn(curin,curout) / curin * (i == 1 ? 0.0 : jointinitscale)
      a = zeros(curin)
      b = zeros(curout)
...
Computes the mean emprirical loglikelihood for the data set `x`.
The probability of a sample is estimated by the empirical probability of the
